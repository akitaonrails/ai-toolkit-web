...
| [LLM Benchmark v4: Opus 5.5, GPT 6...](https://akitaonrails.com/en/2026/09/23/llm-benchmark-v4-opus-5-5-gpt-6-sol-luna-mimo-2-6-grok-4-7/) | Latest round: 5 models on 2026-09-23, bringing the table to 44 models ("The Updated Table, 44 Models"). No new chart; the site keeps the Part 2 chart | `articles.ts` `benchmark.models` and `benchmark.latestRound` |
...
| `content/about.en.md` | Blogging since 2006; X account is protected per the blog, but the owner said on 2026-09-23 that none of his social accounts are protected (the site no longer says so); YouTube channel finished, still valuable; no sponsorships | `common.json` `social`, `writing.json` `hero` |
...
| The SBT News segment the owner sent on 2026-09-25 (`videos.config.json` `tv`) | Title, channel, date, duration, views; summary from the video's description (UN General Assembly on AI risks, AI companies' warnings, global security, AI and deepfakes in the Brazilian elections) | `videos.json` `tv` (generated), shown at the top of `/podcasts/` under "On TV"; `podcasts.json` `items` |
...
| Source | Lands in |
|---|---|
| Owner's instructions (2026-09-23): to use distrobox-gaming, clone the repo, open Claude Code in it and ask for the container and the games you want; you provide your own path with ROMs and other binaries | `games.json` `use`, the "How to use it" section on `/games/` |
...
**Section 4, the scary stories (added 2026-09-26 at the owner's request).** His framing: the reported "AI attacks" are human decisions (a door left open, a tool pointed at a target), not AI out of control, and state hacking is decades old. Hugging Face and RubyGems come from his 2026-09-09 post and the OpenAI incident report it links; Replit from his 2026-07-11 post on agents deleting things and Fortune; the Chinese espionage case and the "blackmail" scenarios from Anthropic's own write-ups; Stuxnet from Wired (Zetter); North Korea from the DOJ charges (via PBS) and the FBI Bybit alert; China from Nextgov (OPM) and the FBI/CISA statement (Salt Typhoon). The quote is his X post https://x.com/AkitaOnRails/status/2103884967168065543, first lines only, as he asked; the gun-maker line is his usual saying.

**Section 5, the doom (added 2026-09-26 at the owner's request).** His framing: the "I'm ex-lab" warnings bring feelings and forecasts, no checkable evidence; don't trust anybody. The Coxon facts come from his 2026-09-09 post as updated on 2026-09-25 (vesting, the Pirate Wires report on the PR firm DEY.). Keep the hedges: the amplification timing ("within minutes", "WSJ 18 minutes earlier") comes from Parker Thayer's thread and is not independently confirmed; the PR-firm booking rests on one email and two unnamed sources. The relationship map (`src/components/AgiNetwork.astro`) is drawn in code, not generated, so every name is exact and every label translatable. There is no diagram by David Sacks: he made the argument in text and on All-In; the map in the owner's post is by @Hesamation. Ours keeps only edges a source supports: solid for public records (Good Ventures funds Coefficient Giving, ex-Open Philanthropy, renamed November 2025; Karnofsky co-founded it, married Daniela Amodei in 2017 and joined Anthropic in 2025; Tallinn led Anthropic's Series A with Moskovitz investing personally; SFF grants to Encode, AI Futures Project and AIPI), dashed for reported ones (the amplification, DEY. booking interviews, the 2022 Good Ventures scholarship to Coxon). Left out: "Open Philanthropy was an early Anthropic investor" (wrong, it was Moskovitz personally) and the Irregular $6.8M link (unconfirmed). Two things in the owner's own post worth fixing there: NBC quotes Benton as "merely blistering", not "frenetic", and the Hugging Face remark was Josh Engels's.
